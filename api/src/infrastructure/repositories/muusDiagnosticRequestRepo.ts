...
import {MuusDiagnosticRequest} from '../entity/MuusDiagnosticRequest';
...
@EntityRepository(MuusDiagnosticRequest)
export class MuusDiagnosticRequestRepository extends Repository<MuusDiagnosticRequest> {
    public async findNonCancelledRequestsIds(chatId: number): Promise<number[]> {
        const connection = getConnection();
            
        const res = await connection
            .createQueryBuilder(MuusDiagnosticRequest, 'req')
            .select('req.id', 'id')
    ...
    public async cancelRequestByIds(ids: number[]): Promise<void> {
        ...
        for (const id of ids) {
            const entity = new MuusDiagnosticRequest();
            entity.id = id;
    ...
    public async findNonCancelledRequest(chatId: number): Promise<MuusDiagnosticRequest> {
        const connection = getConnection();
        const res = await connection
            .createQueryBuilder(MuusDiagnosticRequest, 'req')
